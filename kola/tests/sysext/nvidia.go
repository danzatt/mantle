...
const (
	...
	NvidiaDriversVersion = "550-open"                         // NVIDIA drivers sysext version used in the template
...
storage:
  ...
  - path: /etc/flatcar/enabled-sysext.conf
    contents:
      inline: |
        nvidia-drivers-{{ .NvidiaDriversVersion }}
  links:
`
...
var plog = capnslog.NewPackageLogger("github.com/flatcar/mantle", "kola/tests/sysext")
...
func init() {
	...
	register.Register(&register.Test{
		Name:          "sysext.nvidia.nvidia-operator",
...
func verifyNvidiaInstallation(c cluster.TestCluster) {
	...
	params := map[string]string{
		"NvidiaDriversVersion": NvidiaDriversVersion,
	}
	...
	butane, err := testsutil.ExecTemplate(nvidiaTemplate, params)
	if err != nil {
...
func verifyNvidiaGpuOperator(c cluster.TestCluster) {
	...
	nvidia_smi_out := c.MustSSH(m, "/usr/bin/nvidia-smi")
	c.Logf("nvidia-smi: %s", nvidia_smi_out)
